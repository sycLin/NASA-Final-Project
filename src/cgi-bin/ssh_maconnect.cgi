...
class ProcInfo:
	def __init__(self, init_list):
		...
		self.command = init_list[1]
		self.cpu = float(init_list[2]) # percentage of cpu use
		self.memory = float(init_list[4]) # percentage of memory use
		self.user = init_list[6]
		self.state = init_list[5] # R(running), S(sleeping), Z(zombie)
...
success = ssh.SendReqExec(channelNum,"top -l 1 | tr -s ' ' | cut -d ' ' -f 1,28,13,3,8,4,2")
if (success != True):
    print(ssh.lastErrorText())
    sys.exit()
